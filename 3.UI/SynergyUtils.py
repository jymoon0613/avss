...
def optimize(cap_pred, params_now, n_used) :
    ...
    if len(cons2_ind[0]) == 0 :
        ...
        cons2_ind = np.where(table[:,6] - cap_pred <= 97)
    ...
    # 결과 저장
    x1_target, x2_target, x3_target, x4_target, x5_target, tap_target, cap, cost = table[optimized_ind]

    n_used[0] += np.abs(x1_target - x1_now)
    n_used[1] += np.abs(x2_target - x2_now)
    n_used[2] += np.abs(x3_target - x3_now)
    n_used[3] += np.abs(x4_target - x4_now)
    n_used[4] += np.abs(x5_target - x5_now)
    n_used[5] += np.abs(tap_target - tap_now)

    # 결과 반환
    # 일반 리액터 1, 2, 3, 4, 5 조정 방안 (0 or 1) / 가변형 리택터 1 조정 방안 (0 ~ 18), 최적화 진행 후 투입 용량
    return x1_target, x2_target, x3_target, x4_target, x5_target, tap_target, cap, cost, n_used
